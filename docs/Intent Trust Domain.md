...
| **Type** | **Core field** | **Core field** | **Description** |
| --- | --- | --- | --- |
| `IMMEDIATE` | IntentRaw | raw | Original intent content |
| `IMMEDIATE` | Checkout | totalAmount | Total amount of products |
| `IMMEDIATE` | Checkout | totalCurrency | Currency of the total amount |
| `IMMEDIATE` | Checkout | Merchant | Merchant information |
| `IMMEDIATE` | Checkout | Goods | Product information |
| `AUTONOMOUS` | IntentRaw | raw | Original intent content |
| `AUTONOMOUS` | IntentConstraints | budgetAmount | Total budget amount for a Mandate |
| `AUTONOMOUS` | IntentConstraints | budgetCurrency | Currency of the total budget amount |
| `AUTONOMOUS` | IntentConstraints | merchantNameList | Merchant list authorized for a Mandate |
| `AUTONOMOUS` | IntentConstraints | merchantMccList | MCC list of Merchants authorized for a Mandate |
...
| --- | --- | --- |
| Commercial Transaction | Provides supplementary transaction constraints that enable participants to determine whether a transaction is consistent with the User's authorization. | • Maximum amount per-transaction    • Minimum amount per transaction    • Specified region or country of the Merchants    • Merchant allowlist    • Merchant blocklist    • Delivery address information |
| Agent Behavior | Defines behavioral constraints or handling rules applicable to the Agent during task execution and payment processing. | • Payment frequency limit during the Mandate validity period    • Handling rules after payment failure    • Maximum retry attempts after failure |
...
For details, refer to [S1 Mandate Creation Pre-Verification](Intent%20Trust%20Domain.md#s1-mandate-creation-pre-verification) and [L2 Mandate Authorization Assurance Data](Intent%20Trust%20Domain.md#l2-mandate-authorization-assurance-data).
...
![](images/Figure%208.%20Three-layer%20Assurance%20Data.png)
...
| **L1** | Header | `typ` | string | **MUST** | SD-JWT type: `sd+jwt` |
| **L1** | Header | `kid` | string | **MUST** | Issuer signing key ID |
| **L1** | Payload | `iss` | string | **MUST** | Issuer URI from Alipay+. This URI supports SD-JWT iss domain. |
| **L1** | Payload | `sub` | string | **MUST** | User identifier |
| **L1** | Payload | `iat` | integer | **MUST** | Issuance time |
| **L1** | Payload | `exp` | integer | **MUST** | Expiration time, which must be set to a sufficiently long period. |
| **L1** | Payload | `cnf` | object | **MUST** | ISP Public Key used for binding ISP identity |
| **L1** | Payload | `isp_type` | string | **MUST** | ISP identity type, e.g., `MPP` |
| **L1** | Payload | `isp_name` | string | **MUST** | ISP identity name, e.g., `ALIPAY_HK` |
| **L1** | Payload | `_sd_alg` | string | **MUST NOT** | Disclosure signing algorithm: `sha-256` |
| **L1** | Payload | `_sd` | array | **MUST NOT** | Disclosure hash array |
| **L2** | Header | `alg` | string | **MUST** | Signing algorithm: `ES256` |
| **L2** | Header | `typ` | string | **MUST** | SD-JWT type: `sd+jwt`; subject to extension |
| **L2** | Header | `kid` | string | **MUST** | ISP signing key ID. This string MUST match `cnf.kid` in L1 message. |
| **L2** | Payload | `nonce` | string | **MUST** | Random number for replay protection |
| **L2** | Payload | `aud` | string | **MUST** | Verifier identifier |
| **L2** | Payload | `iat` | integer | **MUST** | Issuance time |
| **L2** | Payload | `exp` | integer | **MUST** | Expiration time |
| **L2** | Payload | `sd_hash` | string | **MUST** | Binds to L1 |
| **L2** | Payload | `idv_result` | string | **MUST** | ISP's identity verification result for the User |
| **L2** | Payload | `idv_time` | string | **MUST** | Time when ISP completes User identity verification |
| **L2** | Payload | `mode` | string | **MUST** | Supports `IMMEDIATE` and `AUTONOMOUS` |
| **L2** | Payload | `_sd_alg` | string | **MUST** | Disclosure signing algorithm: `sha-256` |
| **L2** | Payload | `_sd` | array | **MUST** | Disclosure hash array |
| **L2** | SD Disclosure | `intent` | object | **MUST** | Original intent information |
| **L2** | SD Disclosure | `intent.raw` | string | **MUST** | Original intent |
| **L2** | SD Disclosure | `intent.desc` | string | **MUST** | Original description |
| **L2** | SD Disclosure | `token_info` | object | **MUST** | Payment Token information |
| **L2** | SD Disclosure | `mandate_info` | object | **MUST** | Mandate authorization information |
| **L2** | Payload for `IMMEDIATE` Mandate | `cnf` | object | **MUST NOT** | Binds Agent Public Key |
| **L2** | SD Disclosure for `IMMEDIATE` Mandate | `intent.expiry_time` | string | **MUST NOT** | Intent expiration time |
| **L2** | SD Disclosure for `IMMEDIATE` Mandate | `intent.constraints` | object | **MUST NOT** | Delegated payment constraint information |
| **L2** | SD Disclosure for `IMMEDIATE` Mandate | `checkout` | object | **MUST** | Checkout information |
| **L2** | Payload for `AUTONOMOUS` Mandate | `cnf` | object | **MUST** | Binds Agent Public Key |
| **L2** | SD Disclosure for `AUTONOMOUS` Mandate | `intent.expiry_time` | string | **MUST** | Intent expiration time |
| **L2** | SD Disclosure for `AUTONOMOUS` Mandate | `intent.constraints` | object | **MUST** | Delegated payment constraint information |
| **L2** | SD Disclosure for `AUTONOMOUS` Mandate | `checkout` | object | **MUST NOT** | Checkout information |
| **L3** | Header | `alg` | string | **MUST** | Signing algorithm: `ES256` |
| **L3** | Header | `typ` | string | **MUST** | SD-JWT type: `sd+jwt` |
| **L3** | Header | `kid` | string | **MUST** | This string MUST match `cnf.kid` in L2 message. |
| **L3** | Payload | `nonce` | string | **MUST** | Random number for replay protection |
| **L3** | Payload | `aud` | string | **MUST** | Verifier identifier: payment network URI |
| **L3** | Payload | `iat` | integer | **MUST** | Issuance time |
| **L3** | Payload | `exp` | integer | **MUST** | Expiration time |
| **L3** | Payload | `sd_hash` | string | **MUST** | Binds to L2 |
| **L3** | Payload | `_sd_alg` | string | **MUST** | Disclosure signing algorithm: `sha-256` |
| **L3** | Payload | `_sd` | array | **MUST** | Disclosure hash array |
| **L3** | SD Disclosure | `checkout` | object | **MUST** | Selectively disclosed `checkout` information |
| **L3** | SD Disclosure | `checkout.total_amount` | object | **MUST** | Selectively disclosed `checkout.total_amount` information |
...
![](images/Figure%209.%20Three-layer%20Assurance%20Data%20verification.png)
