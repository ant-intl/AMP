...
# ---------------------------------------------------------------------------
# agent (shopping-agent server — provides /task endpoints for the web frontend)
...
# declare human not present flow
export FLOW=human_not_present
# IDV deferred mode: enrollment/mandate IDV is completed by the user on the
# IDV card (MPP waits for /completeIdv), not auto-completed.
export IDV_DEFERRED=true
...
start_service "shopping-agent" "$PROJECT_ROOT/samples/python/shopping_agent" \
...
# AlipayPlus Network (AlipayPlus — orchestration layer)
...
echo ""
echo "All services started successfully. (FLOW=human_not_present → AUTONOMOUS mode)"
